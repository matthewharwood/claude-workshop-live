...
const Help = () => (
    <Box flexDirection="column" padding={1}>
        <Text bold color="cyan">🚀 JSONL Formatter Agent</Text>
        <Text dimColor>━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━</Text>
        <Box height={1} />

        <Text bold color="yellow">DESCRIPTION:</Text>
        <Text>    Demonstrates various ways to format and extract data from <Text color="cyan">Claude's JSONL output</Text></Text>
        <Text>    using <Text color="green">jq filters</Text>. Showcases <Text color="magenta">8 different recipes</Text> for processing Claude's JSONL</Text>
        <Text>    stream output, including human-readable summaries, tool analysis, cost tracking,</Text>
        <Text>    and more.</Text>
        <Box height={1} />

        <Text bold color="yellow">USAGE:</Text>
        <Text>    <Text color="cyan">bun run agents/jsonl-formatter.ts</Text> [prompt] [options]</Text>
        <Text>    <Text color="cyan">./bin/jsonl-formatter</Text> [prompt] [options]</Text>
        <Box height={1} />

        <Text bold color="yellow">ARGUMENTS:</Text>
        <Text>    <Text color="green">prompt</Text>                    Your prompt to send to Claude (optional)</Text>
        <Box height={1} />

        <Text bold color="yellow">OPTIONS:</Text>
        <Text>    <Text color="green">-r, --recipe &lt;number&gt;</Text>     Run specific recipe (1, 2, 3, 4, 5, 7, 8, 12) or 'all'</Text>
        <Text>    <Text color="green">-v, --verbose</Text>             Enable verbose output (shows raw JSONL data)</Text>
        ...
        <Text>    <Text color="cyan">./bin/jsonl-formatter</Text> "hello world"</Text>
        <Box height={1} />
        <Text dimColor>    # Run specific recipe</Text>
        <Text>    <Text color="cyan">./bin/jsonl-formatter</Text> "explain quantum computing" <Text color="green">--recipe=1</Text></Text>
        <Text>    <Text color="cyan">./bin/jsonl-formatter</Text> "what's the weather?" <Text color="green">-r 3</Text></Text>
        <Box height={1} />
        <Text dimColor>    # Run with verbose output to see raw JSONL data</Text>
        <Text>    <Text color="cyan">./bin/jsonl-formatter</Text> "test prompt" <Text color="green">--verbose --recipe=2</Text></Text>
        <Box height={1} />
        <Text dimColor>    # Get help</Text>
        <Text>    <Text color="cyan">./bin/jsonl-formatter</Text> <Text color="green">--help</Text></Text>
        <Box height={1} />

        <Text bold color="yellow">AVAILABLE RECIPES:</Text>
        <Text>    <Text color="magenta">1:</Text>  Assistant's Text Reply     - Extract the assistant's text response (raw)</Text>
        <Text>    <Text color="magenta">2:</Text>  Session Summary           - Summarize session, models, token totals, and cost</Text>
        <Text>    <Text color="magenta">3:</Text>  MCP Servers Table         - Turn MCP server statuses into a Markdown table</Text>
        <Text>    <Text color="magenta">4:</Text>  Tool Counts               - Count tools and slash commands</Text>
        <Text>    <Text color="magenta">5:</Text>  Tools by Origin           - Tools grouped by origin (core vs MCP providers)</Text>
        <Text>    <Text color="magenta">7:</Text>  Timeline Summary          - Human-readable one-line summary per event</Text>
        <Text>    <Text color="magenta">8:</Text>  Duration Analysis         - Convert durations to seconds with service tier info</Text>
        <Text>    <Text color="magenta">12:</Text> Human Sentence            - Emit a single human-readable sentence</Text>
        <Box height={1} />

        <Text bold color="yellow">NOTES:</Text>
        <Text>    <Text dimColor>•</Text> Pass-through arguments: Any unrecognized arguments are passed to the claude command</Text>
        <Text>    <Text dimColor>•</Text> Default prompt: If no prompt is provided, uses a default about JSONL formatting</Text>
        <Text>    <Text dimColor>•</Text> Output formats: Some recipes produce raw text, others produce formatted JSON</Text>
    </Box>
);
...
// React Components
const RecipeOutput = ({ recipe, number, jsonlData }: { recipe: Recipe; number: string; jsonlData: string }) => {
    ...
    return (
        <Box flexDirection="column" marginY={1}>
            <Box flexDirection="column" padding={1} borderStyle="round" borderColor="cyan">
                <Text bold color="magenta">📊 RECIPE {number}: {recipe.name}</Text>
    ...
    return (
        <Box flexDirection="column">
            <Box flexDirection="column" padding={1}>
                <Text bold color="cyan">🚀 JSONL Formatter Agent</Text>
                <Text dimColor>━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━</Text>
                <Text>Running Claude with prompt: <Text color="cyan">"{userPrompt}"</Text></Text>
                <Text>Format option: <Text color="green">{formatOption as string}</Text></Text>
            </Box>
            ...
            {loading ? (
                ...
                <Box flexDirection="column">
                    <Box padding={1}>
                        <Text color="green">✅ Received {jsonlData.split("\n").length} lines of JSONL data</Text>
                    </Box>
                    ...
                    {formatOption === "all" ? (
                        ...
                        <Box flexDirection="column" padding={1}>
                            ...
                            {Object.entries(recipes).map(([number, recipe]) => (
                                <Text key={number}>  {number}: {recipe.name}</Text>
                            ))}
                    ...
                    <Box padding={1}>
                        <Text dimColor>━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━</Text>
                        <Text bold color="green">✅ JSONL formatting demonstration complete!</Text>
                        <Text dimColor>━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━</Text>
                    </Box>
                </Box>
            )}
